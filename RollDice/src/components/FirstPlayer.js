...
import {Players, row} from '../styles/gameStyles';
    ...
    switch (whichOne) {
      case 1:
        if (this.state.player1.position11 === position) {
          return <Icon name="user" color="red" size={20} />;
        }
        break;
      case 2:
      ...
      <View style={[styles.player1, Players.styles]}>
        <View style={row.Style}>
          <View style={styles.places}>
            <TouchableOpacity>{this.checkPostion(1, 1, -11)}</TouchableOpacity>
          </View>
          <View style={styles.places}>
            <TouchableOpacity>{this.checkPostion(1, 2, -21)}</TouchableOpacity>
          </View>
        </View>
        <View style={row.Style}>
          <View style={styles.places}>
            <TouchableOpacity>{this.checkPostion(1, 3, -31)}</TouchableOpacity>
          </View>
          <View style={styles.places}>
            <TouchableOpacity>{this.checkPostion(1, 4, -41)}</TouchableOpacity>
          </View>
        </View>
      </View>
    );
